...
const getAboutMeInfo = async (): Promise<HandlerResult<AboutMeInfoResponseType>> => {
    try {
        ...
        return { data: { ...aboutPage, avatarUrl }, msg: '成功' };
    } catch (error) {
        throw error;
    }
}
 ...
 * 修改关于我信息
 */
const updateAboutMeInfo = async (param: ParameBodyType<UpdateAboutMeInfoParamsType>): Promise<HandlerResult<null>> => {
    ...
    try {
        ...
        if (param.isUpdateAvatar && param.avatarCode) {
            ...
            const avatarItem = confirmResult.data.find((item: ConfirmResultType) => item.fileCode === param.avatarCode);
            if (!avatarItem) {
                await transaction.rollback();
                return { err: '未找到头像文件' };
            }
