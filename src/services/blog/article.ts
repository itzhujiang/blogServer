...
import { ArticleAttributes, CategoryAttributes, Article, Category, ArticleStatusLiteral } from '../../models/index';
...
type ArticleListRequsetType = {
    ...
    status?: ArticleStatusLiteral;      // 文章状态
    publishedAtStart?: number;          // 发布时间-开始（毫秒时间戳）
    publishedAtEnd?: number;            // 发布时间-结束（毫秒时间戳）
    categoryId?: number;                // 分类ID
    viewCountSort?: 'desc' | 'asc';     // 浏览量排序
};

/**
 * 文章列表项类型
 */
type ArticleItemType = Pick<ArticleAttributes,
    'id' | 'title' | 'slug' | 'excerpt' | 'thumbnailUrl' |
    'authorName' | 'readingTime' | 'viewCount' | 'status' | 'publishedAt'
> & {
 ...
 * @returns 文章列表
 */
const getArticleList = async (param: ParameBodyType<ArticleListRequsetType>): Promise<HandlerResult<ArticleListResponseType>> => {
    const {
        page = 1,
        size = 10,
        title,
