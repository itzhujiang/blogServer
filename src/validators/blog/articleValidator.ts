...
export const getArticleListValidation = [
  ...
  query('status')
    .optional()
    .isIn(['draft', 'published', 'archived']).withMessage('状态必须是 draft/published/archived'),

  // 发布时间范围
  query('publishedAtStart')
    .optional()
    .isInt({ min: 0 }).withMessage('开始时间必须是有效的时间戳')
    .toInt(),
  query('publishedAtEnd')
    .optional()
    .isInt({ min: 0 }).withMessage('结束时间必须是有效的时间戳')
    .toInt(),
  ...
  query('viewCountSort')
    .optional()
    .isIn(['asc', 'desc']).withMessage('排序方式必须是 asc 或 desc')
];