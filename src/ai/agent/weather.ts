...
import { getWeather, getIpPosition, generalTools } from '../tools';
...
export const getWeatherAgentNodes = () => {
  const staticTools = [getWeather, getIpPosition, ...generalTools];
...
7. 关于天气的展示，如有天气展示工具，可以直接使用天气展示工具，如果没有则通过文字告诉用户\n\n${state.memoryPrompt ?? ''}`,
      },
      ...state.messages,
    ]);
    return {
